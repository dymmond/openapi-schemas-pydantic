...
def test_media_type_alias() -> None:
    ...
    media_type_2 = MediaType.model_validate({"media_type_schema": Schema()})
    media_type_3 = MediaType.model_validate({"schema": Schema()})
...
def test_parameter_alias() -> None:
    ...
    parameter_2 = Parameter.model_validate(
        {"name": "test", "param_in": "path", "param_schema": Schema()}
    )
    parameter_3 = Parameter.model_validate({"name": "test", "in": "path", "schema": Schema()})
...
def test_security_scheme() -> None:
    ...
    security_scheme_2 = SecurityScheme.model_validate(
